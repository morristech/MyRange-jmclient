...
public class PNG
{
    ...
    public static byte[] imageToPNG(Image image)
    {
        try
        {
            ...
            rgbArray = image.getRGB();

            for(int i=0; i<rgbArray.length; i++)
            {
               alpha[i] = (byte) (rgbArray[i] >> 24);
               red[i] = (byte) ((rgbArray[i] >> 16) & 0x000000FF);
               green[i] = (byte) ((rgbArray[i] >>  8) & 0x000000FF);
               blue[i] = (byte) (rgbArray[i] & 0x000000FF);
            }
